...
#if defined(CONFIG_MTK_FPGA) //defined(CONFIG_POWER_EXT)
//----------------------------------------------------------------------
#define CUST_EINT_MT6322_PMIC_NUM              25
#define CUST_EINT_MT6322_PMIC_DEBOUNCE_CN      1
#define CUST_EINT_MT6322_PMIC_TYPE						 4
#define CUST_EINT_MT6322_PMIC_DEBOUNCE_EN      1
//----------------------------------------------------------------------
#else
#include <cust_eint.h>
#endif
...
#define EXT_BUCK_HV_THD   0x78E //3.4V
#define EXT_BUCK_LV_1_THD 0x738 //3.25V
#define EXT_BUCK_LV_2_THD 0x6E3 //3.1V
...
void low_battery_protect_init(void)
{
    ...
    //LV interrupt volt and en setting in ext_buck_init()
    //HV interrupt volt and en setting in ext_buck_init()   
...
void bat_l_int_handler(void)
{
    ...
    if( is_ext_buck_exist()==1 ) {
        upmu_set_rg_lbat_volt_min(EXT_BUCK_LV_2_THD);
    } else {
...
void bat_h_int_handler(void)
{
    ...
    #else
    if( is_ext_buck_exist()==1 ) {
        upmu_set_rg_lbat_volt_min(EXT_BUCK_LV_1_THD);
    } else {
...
void chrdet_int_handler(void)
{
    ...
    //enable charger current(>200mA) detect
...
static int pmic_thread_kthread(void *x)
{
    ...
    while (1) {
        ...
        mt_eint_unmask(CUST_EINT_MT6322_PMIC_NUM);
...
{
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[mt_pmic_eint_irq] receive interrupt\n");
...
{
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC_EINT_SETTING] start: CUST_EINT_PMIC_NUM=%d\n",CUST_EINT_MT6322_PMIC_NUM);

    //ON/OFF interrupt
    cust_pmic_interrupt_en_setting();
    
    mt_eint_set_hw_debounce(    CUST_EINT_MT6322_PMIC_NUM,
                                CUST_EINT_MT6322_PMIC_DEBOUNCE_CN);     // set debounce time
    mt_eint_registration(       CUST_EINT_MT6322_PMIC_NUM,                                
                                CUST_EINT_MT6322_PMIC_TYPE,
                                mt_pmic_eint_irq,
                                0);

    mt_eint_unmask(CUST_EINT_MT6322_PMIC_NUM);

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[CUST_EINT] CUST_EINT_PMIC_NUM=%d\n", CUST_EINT_MT6322_PMIC_NUM);
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[CUST_EINT] CUST_EINT_PMIC_DEBOUNCE_CN=%d\n", CUST_EINT_MT6322_PMIC_DEBOUNCE_CN);
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[CUST_EINT] CUST_EINT_PMIC_TYPE=%d\n", CUST_EINT_MT6322_PMIC_TYPE);
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[CUST_EINT] CUST_EINT_PMIC_DEBOUNCE_EN=%d\n", CUST_EINT_MT6322_PMIC_DEBOUNCE_EN);
...
void pmic_ldo_enable(MT65XX_POWER powerId, kal_bool powerEnable)
{
    ...
    else if(powerId == MT6322_POWER_LDO_VIO28)      { dct_pmic_VIO28_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VUSB)       { dct_pmic_VUSB_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VMC)        { dct_pmic_VMC_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VMCH)       { dct_pmic_VMCH_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VEMC_3V3)   { dct_pmic_VEMC_3V3_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VGP1)       { dct_pmic_VGP1_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VGP2)       { dct_pmic_VGP2_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VGP3)       { dct_pmic_VGP3_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VCN_1V8)    { dct_pmic_VCN_1V8_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VSIM1)      { dct_pmic_VSIM1_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VSIM2)      { dct_pmic_VSIM2_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VRTC)       { dct_pmic_VRTC_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VCAM_AF)    { dct_pmic_VCAM_AF_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VIBR)       { dct_pmic_VIBR_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VM)         { dct_pmic_VM_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VRF18)      { dct_pmic_VRF18_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VIO18)      { dct_pmic_VIO18_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VCAMD)      { dct_pmic_VCAMD_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VCAM_IO)    { dct_pmic_VCAM_IO_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VTCXO)      { dct_pmic_VTCXO_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VA)         { dct_pmic_VA_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VCAMA)      { dct_pmic_VCAMA_enable(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VCN33_BT)   { dct_pmic_VCN33_enable_bt(powerEnable); }
    else if(powerId == MT6322_POWER_LDO_VCN33_WIFI) { dct_pmic_VCN33_enable_wifi(powerEnable); }	
    else if(powerId == MT6322_POWER_LDO_VCN28)      { dct_pmic_VCN28_enable(powerEnable); }
...
void pmic_ldo_vol_sel(MT65XX_POWER powerId, MT65XX_POWER_VOLTAGE powerVolt)
{
    ...
    else if(powerId == MT6322_POWER_LDO_VIO28)      { xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VIO28_enable] No vlotage can setting!\n"); }
    else if(powerId == MT6322_POWER_LDO_VUSB)       { xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VUSB_enable] No vlotage can setting!\n"); }
    else if(powerId == MT6322_POWER_LDO_VMC)        { dct_pmic_VMC_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VMCH)       { dct_pmic_VMCH_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VEMC_3V3)   { dct_pmic_VEMC_3V3_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VGP1)       { dct_pmic_VGP1_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VGP2)       { dct_pmic_VGP2_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VGP3)       { dct_pmic_VGP3_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VCN_1V8)    {xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VCN_1V8_enable] No vlotage can setting!\n");}
    else if(powerId == MT6322_POWER_LDO_VSIM1)      { dct_pmic_VSIM1_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VSIM2)      { dct_pmic_VSIM2_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VRTC)       { xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VRTC_enable] No vlotage can setting!\n");}
    else if(powerId == MT6322_POWER_LDO_VCAM_AF)    { dct_pmic_VCAM_AF_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VIBR)       { dct_pmic_VIBR_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VM)         { dct_pmic_VM_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VRF18)      { xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VRF18_enable] No vlotage can setting!\n");}
    else if(powerId == MT6322_POWER_LDO_VIO18)      { xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VIO18_enable] No vlotage can setting!\n");}
    else if(powerId == MT6322_POWER_LDO_VCAMD)      { dct_pmic_VCAMD_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VCAM_IO)    { xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VCAM_IO_enable] No vlotage can setting!\n");}
    else if(powerId == MT6322_POWER_LDO_VTCXO)      { xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VTCXO_enable] No vlotage can setting!\n");}
    else if(powerId == MT6322_POWER_LDO_VA)         { xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VA_enable] No vlotage can setting!\n"); }
    else if(powerId == MT6322_POWER_LDO_VCAMA)      { dct_pmic_VCAMA_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VCN33_BT)   { dct_pmic_VCN33_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VCN33_WIFI) { dct_pmic_VCN33_sel(powerVolt); }	
    else if(powerId == MT6322_POWER_LDO_VCN28)      { xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VCN28_enable] No vlotage can setting!\n"); }
...
static int proc_utilization_show(struct seq_file *m, void *v)
{
    ...
    seq_printf(m, "********** ldo status dump seq_printf [1:ON,0:OFF]**********\n");
...
extern int is_ncp6335_sw_ready(void);
extern int is_ncp6335_exist(void);
extern int ncp6335_vosel(unsigned long val);
extern int get_ncp6335_i2c_ch_num(void);
...
int get_ext_buck_i2c_ch_num(void)
{
    ...
    {
        return get_fan53555_i2c_ch_num();
    }
    else if(is_ncp6335_exist()==1)
    {
        return get_ncp6335_i2c_ch_num();
    }
    else
    {
        return -1;
...
{    
    if( (is_fan53555_sw_ready()==1) && (is_ncp6335_sw_ready()==1) )
        return 1;    
    else
        return 0;    
}
...
int is_ext_buck_exist(void)
{
    if( (is_fan53555_exist()==1) || (is_ncp6335_exist()==1) )
        return 1;
    else
        return 0;    
}
...
int ext_buck_vosel(unsigned long val)
{
    ...
    if(is_ext_buck_sw_ready()==1)
    {
        if(is_fan53555_exist()==1)
        {
            ret = fan53555_vosel(val);
        }
        else if(is_ncp6335_exist()==1)
        {
            ret = ncp6335_vosel(val);
        }
        else
        {
...
void ext_buck_init_for_platform(void)
{
    ...
    if( (g_vproc_vsel_gpio_number!=0) && (g_vproc_en_gpio_number!=0) )
    {
        pmic_config_interface(0x048,0x1,0x1,0);
        pmic_config_interface(0x210,0x0,0x1,9);
        pmic_config_interface(0x21A,0x0,0x1,0);
        pmic_config_interface(0x216,0x0,0x1,0); // sw mode
        xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[ext_buck_init_for_platform] turn off internal VPROC.\n");
    }
    else
    {
        #ifdef IS_VCORE_USE_6333VCORE
            xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[ext_buck_init_for_platform] IS_VCORE_USE_6333VCORE=y, nothing for internal VPROC.\n");
            //add setting id needed
        #else
            xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[ext_buck_init_for_platform] IS_VCORE_USE_6333VCORE=n, nothing for internal VPROC.\n");
            //add setting id needed
        #endif
    }
}
...
void ext_buck_init(void)
{
    if( is_ext_buck_exist()==1 )
    {
        ...
        #ifdef LOW_BATTERY_PROTECT
        upmu_set_rg_lbat_volt_max(EXT_BUCK_HV_THD);
        upmu_set_rg_lbat_volt_min(EXT_BUCK_LV_1_THD);
        #endif
...
void ext_buck_pre_init(void)
{
    ...
    if(g_vproc_vsel_gpio_number!=0)
    {
        if(g_vproc_en_gpio_number!=0)
        {
            pmic_config_interface(0x220,0x28,0x7F,0); // internal VPROC=0.95
            xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[ext_buck_pre_init] Reg[0x220]=0x%x\n", upmu_get_reg_value(0x220));
            
            ext_buck_vproc_en(1);
            xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[ext_buck_pre_init] ext_buck_vproc_en(1).\n");
        }
        else
        {
            xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[ext_buck_pre_init] g_vproc_en_gpio_number==0\n");
        }
    }
    else
    {
        xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[ext_buck_pre_init] g_vproc_vsel_gpio_number==0\n");
    }
#else
    if(g_vproc_vsel_gpio_number!=0)
    {
        ext_buck_vproc_vsel(1); 
        xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[ext_buck_pre_init] ext_buck_vproc_vsel(1)\n");
    }
...
void PMIC_INIT_SETTING_V1(void)
{
    ...
    if(chip_version >= PMIC_E2_CID_CODE)
    {
...
ret = pmic_config_interface(0x778,0x1,0x1,15); // [15:15]: RG_VREF18_ENB_MD; 
    ...
    else if(chip_version >= PMIC_E1_CID_CODE)
    {
        xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[Kernel_PMIC_INIT_SETTING_V1] PMIC Chip = %x\n",chip_version);
...
void PMIC_INIT_SETTING_V1_hpm(void)
{
    U32 chip_version = 0;
    U32 ret=0;
    ...
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC_INIT_SETTING_V1_hpm] PMIC Chip = %x\n",chip_version);
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC_INIT_SETTING_V1_hpm] 20130902\n");
    ...
    ret = pmic_config_interface(0x120,0x1,0x1,4); // [4:4]: RG_SRCLKEN_HW_MODE; 
    ret = pmic_config_interface(0x120,0x1,0x1,5); // [5:5]: RG_OSC_HW_MODE; 
    ret = pmic_config_interface(0x130,0x1,0xFF,8); // [15:8]: RG_TOP_CKTST2_RSV_15_8; 32k_1V8 from 75k/2 switch to 32k
    ret = pmic_config_interface(0x148,0x1,0x1,1); // [1:1]: RG_SMT_INT; 
    ret = pmic_config_interface(0x148,0x1,0x1,3); // [3:3]: RG_SMT_RTC_32K1V8; 
    ret = pmic_config_interface(0x160,0x1,0x1,2); // [2:2]: RG_INT_EN_BAT_L; 
    ret = pmic_config_interface(0x160,0x1,0x1,6); // [6:6]: RG_INT_EN_THR_L; 
    ...
    ret = pmic_config_interface(0x166,0x1,0x1,1); // [1:1]: RG_INT_EN_FCHRKEY; 
    ret = pmic_config_interface(0x212,0x2,0x3,4); // [5:4]: QI_VPROC_VSLEEP; 
    ret = pmic_config_interface(0x216,0x1,0x1,0); // [0:0]: VPROC_EN_CTRL; Enable control by SRCLKEN for 6592 only
    //ret = pmic_config_interface(0x216,0x1,0x1,1); // [1:1]: VPROC_VOSEL_CTRL;  //should be after voltage setting
    ret = pmic_config_interface(0x21C,0x17,0x7F,0); // [6:0]: VPROC_SFCHG_FRATE; 
    ret = pmic_config_interface(0x21C,0x1,0x1,7); // [7:7]: VPROC_SFCHG_FEN; 
    ret = pmic_config_interface(0x21C,0x1,0x1,15); // [15:15]: VPROC_SFCHG_REN; 
    ret = pmic_config_interface(0x21E,0x30,0x7F,0); // [6:0]: VPROC_VOSEL; VPROC normal mode voltage=1.0V for 6592 only
    ret = pmic_config_interface(0x220,0x30,0x7F,0); // [6:0]: VPROC_VOSEL_ON; VPROC normal mode voltage=1.0V for 6592 only
    ret = pmic_config_interface(0x222,0x30,0x7F,0); // [6:0]: VPROC_VOSEL_SLEEP; VPROC sleep mode voltage=1.0V for 6592 only
    ret = pmic_config_interface(0x230,0x3,0x3,0); // [1:0]: VPROC_TRANSTD; 
    ret = pmic_config_interface(0x230,0x0,0x1,8); // [8:8]: VPROC_VSLEEP_EN; sleep mode voltage software mode
    ret = pmic_config_interface(0x238,0x3,0x3,0); // [1:0]: RG_VSYS_SLP; 
    ret = pmic_config_interface(0x238,0x2,0x3,4); // [5:4]: QI_VSYS_VSLEEP; 
    ret = pmic_config_interface(0x23C,0x1,0x1,1); // [1:1]: VSYS_VOSEL_CTRL; after 0x0256
    ret = pmic_config_interface(0x242,0x23,0x7F,0); // [6:0]: VSYS_SFCHG_FRATE; 
    ...
    ret = pmic_config_interface(0x216,0x1,0x1,1); // [1:1]: VPROC_VOSEL_CTRL;  //put here for right init timing
...
void test_ldo_id(void)
{
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VIO28     =%d\n", MT6323_POWER_LDO_VIO28     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VUSB      =%d\n", MT6323_POWER_LDO_VUSB      );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VMC       =%d\n", MT6323_POWER_LDO_VMC       );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VMCH      =%d\n", MT6323_POWER_LDO_VMCH      );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VEMC_3V3  =%d\n", MT6323_POWER_LDO_VEMC_3V3  );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VGP1      =%d\n", MT6323_POWER_LDO_VGP1      );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VGP2      =%d\n", MT6323_POWER_LDO_VGP2      );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VGP3      =%d\n", MT6323_POWER_LDO_VGP3      );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VCN_1V8   =%d\n", MT6323_POWER_LDO_VCN_1V8   );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VSIM1     =%d\n", MT6323_POWER_LDO_VSIM1     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VSIM2     =%d\n", MT6323_POWER_LDO_VSIM2     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VRTC      =%d\n", MT6323_POWER_LDO_VRTC      );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VCAM_AF   =%d\n", MT6323_POWER_LDO_VCAM_AF   );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VIBR      =%d\n", MT6323_POWER_LDO_VIBR      );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VM        =%d\n", MT6323_POWER_LDO_VM        );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VRF18     =%d\n", MT6323_POWER_LDO_VRF18     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VIO18     =%d\n", MT6323_POWER_LDO_VIO18     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VCAMD     =%d\n", MT6323_POWER_LDO_VCAMD     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VCAM_IO   =%d\n", MT6323_POWER_LDO_VCAM_IO   );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VTCXO     =%d\n", MT6323_POWER_LDO_VTCXO     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VA        =%d\n", MT6323_POWER_LDO_VA        );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VCAMA     =%d\n", MT6323_POWER_LDO_VCAMA     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VCN33_BT  =%d\n", MT6323_POWER_LDO_VCN33_BT  );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VCN33_WIFI=%d\n", MT6323_POWER_LDO_VCN33_WIFI);  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_VCN28     =%d\n", MT6323_POWER_LDO_VCN28     );  
    
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VIO28     =%d\n", MT6322_POWER_LDO_VIO28     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VUSB      =%d\n", MT6322_POWER_LDO_VUSB      );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VMC       =%d\n", MT6322_POWER_LDO_VMC       );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VMCH      =%d\n", MT6322_POWER_LDO_VMCH      );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VEMC_3V3  =%d\n", MT6322_POWER_LDO_VEMC_3V3  );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VGP1      =%d\n", MT6322_POWER_LDO_VGP1      );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VGP2      =%d\n", MT6322_POWER_LDO_VGP2      );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VGP3      =%d\n", MT6322_POWER_LDO_VGP3      );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VCN_1V8   =%d\n", MT6322_POWER_LDO_VCN_1V8   );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VSIM1     =%d\n", MT6322_POWER_LDO_VSIM1     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VSIM2     =%d\n", MT6322_POWER_LDO_VSIM2     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VRTC      =%d\n", MT6322_POWER_LDO_VRTC      );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VCAM_AF   =%d\n", MT6322_POWER_LDO_VCAM_AF   );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VIBR      =%d\n", MT6322_POWER_LDO_VIBR      );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VM        =%d\n", MT6322_POWER_LDO_VM        );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VRF18     =%d\n", MT6322_POWER_LDO_VRF18     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VIO18     =%d\n", MT6322_POWER_LDO_VIO18     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VCAMD     =%d\n", MT6322_POWER_LDO_VCAMD     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VCAM_IO   =%d\n", MT6322_POWER_LDO_VCAM_IO   );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VTCXO     =%d\n", MT6322_POWER_LDO_VTCXO     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VA        =%d\n", MT6322_POWER_LDO_VA        );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VCAMA     =%d\n", MT6322_POWER_LDO_VCAMA     );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VCN33_BT  =%d\n", MT6322_POWER_LDO_VCN33_BT  );  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VCN33_WIFI=%d\n", MT6322_POWER_LDO_VCN33_WIFI);  
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_VCN28     =%d\n", MT6322_POWER_LDO_VCN28     );  
    
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6323_POWER_LDO_DEFAULT   =%d\n", MT6323_POWER_LDO_DEFAULT   );
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT6322_POWER_LDO_DEFAULT   =%d\n", MT6322_POWER_LDO_DEFAULT   );
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT65XX_POWER_COUNT_END     =%d\n", MT65XX_POWER_COUNT_END     );
}
...
static int pmic_mt_probe(struct platform_device *dev)
{
    ...
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "MT PMIC CID=0x%x\n", ret_val );
...
#ifdef MTK_FAN5402_SUPPORT
    pmic_config_interface(0x0,0x1,0x1,4); // [6:6]: RG_CHR_EN; 
#endif    
    PMIC_INIT_SETTING_V1_hpm();
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[PMIC_INIT_SETTING_V1_hpm] Done\n");
