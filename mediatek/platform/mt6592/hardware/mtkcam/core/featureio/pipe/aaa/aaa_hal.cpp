...
MBOOL Hal3A::setDebugInfo(IBaseCamExif *pIBaseCamExif) const
{
    ...
    r3ADebugInfo.hdr.u4ModuleCount = MODULE_NUM(6,5); //MODULE_NUM(7,6);
...
MVOID * Hal3A::AFThreadFunc(void *arg)
{
    ...
    #ifdef MTK_SLOW_MOTION_VIDEO
    MINT32 i4SkipAFframe=0;
    static MINT32 i4AFfrmcount=0;
    static MINT32 i4preMinfps = 0;
    #endif
    while (g_bAFThreadLoop) {
        ...
        {    
            MY_LOG_IF(sm_3AThreadLogEnable, "[Hal3A::AFThreadFunc]AF frame rate %d\n",g_AFMinFps);
        ...
        {
            i4AFfrmcount--;
            MY_LOG_IF(sm_3AThreadLogEnable, "[Hal3A::AFThreadFunc] skip AF frame %d\n", i4AFfrmcount);
        }
        else
        {
