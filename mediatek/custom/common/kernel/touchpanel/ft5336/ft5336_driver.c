...
//#include "FT5336_HiKe_Vegeta_OGS_1080X1920_Truly0x5a_Ver0x10_20140307_app.i"
//#include "FT5336_HiKe_Vegeta_OGS_1080X1920_Truly0x5a_Ver0x11_20140331_app.i"
//#include "FT5336_HiKe_Vegeta_OGS_1080X1920_Truly0x5a_Ver0x12_20140403_app.i"
//#include "FT5336_HiKe_Vegeta_OGS_1080X1920_Truly0x5a_Ver0x13_20140507_app.i"
//#include "FT5336_HiKe_Vegeta_OGS_1080X1920_Truly0x5a_Ver0x14_20140529_app.i"
...
static  void tpd_up(int x, int y,int *count) {
 ...
 static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
 {
	...
	#ifdef FTS_PRESSURE
	i2c_smbus_read_i2c_block_data(ft5336_i2c_client, 0x00+32, 1, &(data[32]));
        #endif
		...
		for(i = 0; i < point_num; i++)
		{
	       ...
	       /*get the X coordinate, 2 bytes*/
			...
			cinfo->au8_touch_area[i]=((data[3+6 * i+5]&0xf0) >> 2);//data[3+6 * i+5] >> 4
                            ...
                            #if 0
		printk("ft5336 cinfo->x[0] = %d, cinfo->y[0] = %d, cinfo->au8_touch_weight[0] = %d,cinfo->au8_touch_area[0] = %d\n", cinfo->x[0], cinfo->y[0], cinfo->au8_touch_weight[0],cinfo->au8_touch_area[0]);	
		printk("ft5336 cinfo->x[1] = %d, cinfo->y[1] = %d, cinfo->au8_touch_weight[1] = %d,cinfo->au8_touch_area[1] = %d\n", cinfo->x[1], cinfo->y[1], cinfo->au8_touch_weight[1],cinfo->au8_touch_area[1]);
		printk("ft5336 cinfo->x[2] = %d, cinfo->y[2] = %d, cinfo->au8_touch_weight[2] = %d,cinfo->au8_touch_area[2] = %d\n", cinfo->x[2], cinfo->y[2], cinfo->au8_touch_weight[2],cinfo->au8_touch_area[2]);
		printk("ft5336 cinfo->x[3] = %d, cinfo->y[3] = %d, cinfo->au8_touch_weight[3] = %d,cinfo->au8_touch_area[3] = %d\n", cinfo->x[3], cinfo->y[3], cinfo->au8_touch_weight[3],cinfo->au8_touch_area[3]);
		printk("ft5336 cinfo->x[4] = %d, cinfo->y[4] = %d, cinfo->au8_touch_weight[4] = %d,cinfo->au8_touch_area[4] = %d\n", cinfo->x[4], cinfo->y[4], cinfo->au8_touch_weight[4],cinfo->au8_touch_area[4]);
                           #endif
...
unsigned char ver;
ft5x0x_read_reg(0x8B, &ver);
printk("reg 0x8B value is %d\n",ver);
}
EXPORT_SYMBOL(tp_read_reg8B);
#endif
 static void tpd_resume( struct early_suspend *h )
 {
  //int retval = TPD_OK;
